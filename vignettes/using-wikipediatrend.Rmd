...
## Identifying anomalies with `AnomalyDetection` by Twitter, Inc.
...
## Identifying mean shifts with `BreakoutDetection` by Twitter, Inc.

`BreakoutDetection` is a package that allows search data for mean level shifts
...
occur during the the whole time frame (`method`); and several method specific 
options, e.g. `decree`, `beta`, and `percent` which control the sensitivity 
adding further breakpoints. In the following case the the last option tells 
